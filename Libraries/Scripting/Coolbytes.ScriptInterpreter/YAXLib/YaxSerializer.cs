...
public class YaxSerializer
{
    ...
    /// </summary>
    private readonly YAXExceptionHandlingPolicies m_exceptionPolicy;

    /// <summary>
    /// The list of all errors that have occured.
    /// </summary>
    private readonly YaxParsingErrors m_parsingErrors = new();

    /// <summary>
    /// The exception error behaviour enumeration to be used by the YAX library.
    /// </summary>
    private readonly YAXExceptionTypes m_defaultExceptionType;
...
#pragma warning restore S1450
    ...
    /// </summary>
    private readonly YAXSerializationOptions m_serializationOption;
    ...
    /// </summary>
    private readonly Dictionary<XNamespace, string> m_namespaceToPrefix = [];
    ...
    /// <summary>
    /// Spécifie si on doit rechercher les sous-classes de la classe à sérialiser
    /// </summary>
    public bool Heritage { get; set; }

    /// <summary>
    /// Spécifie quels type de champs doivent être sérializés (public/privé/...)
    /// </summary>
    /// <param name="field">Type de champs</param>
    public void ChangeTypeField(YAXSerializationFields field)
    {
        m_udtWrapper.FieldsToSerialize = field;
    }

    /// <summary>
    /// Spécifie la portée que doit avoir les champs ou les propriétés
    /// Par défaut c'est toutes les public/private que ce soit instance ou static (tout quoi)
    /// </summary>
    /// <param name="nouvellePortee">Nouvelle portée</param>
    public void ChangePortee(BindingFlags nouvellePortee)
    {
        m_udtWrapper.Portee = nouvellePortee;
    }
    ...
    public YaxSerializer(Type t, YAXExceptionHandlingPolicies exceptionPolicy, YAXExceptionTypes defaultExType, YAXSerializationOptions option)
    {
        ...
        YaxLibNamespacePrefix = "yaxlib";
        YAXLibNamespaceUri = XNamespace.Get("http://www.sinairv.com/yaxlib/");
        DimentionsAttributeName = "dims";
        RealTypeAttributeName = "realtype";
    }
    ...
    /// </summary>
    public XNamespace YAXLibNamespaceUri { get; set; }

    /// <summary>
    /// The prefix used for the xml namespace
    /// </summary>
    public string YaxLibNamespacePrefix { get; set; }

    /// <summary>
    /// the attribute name used to deserialize meta-data for multi-dimensional arrays.
    /// </summary>
    public string DimentionsAttributeName { get; set; }
    ...
    /// </summary>
    public string RealTypeAttributeName { get; set; }

    private const int DefaultMaxRecursion = 255;

    /// <summary>
    /// Specifies the maximum serialization depth (default 255).
    ...
    public object Deserialize(string input)
    {
        try
        {
            ...
            XDocument xdoc = XDocument.Load(tr, GetXmlLoadOptions());
        ...
        {
            OnExceptionOccurred(new YaxBadlyFormedXmlException(ex, ex.LineNumber, ex.LinePosition), m_defaultExceptionType);
            return null;
        }
    }
    ...
    public void SetDeserializationBaseObject(object obj)
    {
        ...
        {
            throw new YaxObjectTypeMismatchException(m_type, obj.GetType());
        }
    ...
    private XElement SerializeBase(object obj)
    {
        ...
        {
            XElement elemResult = MakeDictionaryElement(null, m_udtWrapper.Alias, obj,
        ...
        {
            XElement elemResult = MakeCollectionElement(null, m_udtWrapper.Alias, obj, null, null);
            if (m_udtWrapper.PreservesWhitespace)
                XmlUtils.AddPreserveSpaceAttribute(elemResult);
            if (elemResult.Parent == null)
                AddNamespacesToElement(elemResult);
        ...
        {
            XElement elemResult = MakeBaseElement(null, m_udtWrapper.Alias, obj, out _);
            if (m_udtWrapper.PreservesWhitespace)
                XmlUtils.AddPreserveSpaceAttribute(elemResult);
            if (elemResult.Parent == null)
                AddNamespacesToElement(elemResult);
        ...
        else if (!m_udtWrapper.UnderlyingType.EqualsOrIsNullableOf(obj.GetType()))
        {
            ...
            // object of its child
            YaxSerializer ser = NewInternalSerializer(obj.GetType(), TypeNamespace, null);
            XDocument xdoc = ser.SerializeToXDocument(obj);
            ...
            AddMetadataAttribute(elem, YAXLibNamespaceUri + RealTypeAttributeName, obj.GetType().FullName, m_documentDefaultNamespace);
        ...
        else
        {
            ...
            XElement elem = SerializeBase(obj, m_udtWrapper.Alias);
    ...
    private XElement SerializeBase(object obj, XName className)
    {
        ...
        {
            XElement baseElem = new(className, null);
        ...
        if (!m_type.IsValueType())
        {
            object alreadySerializedObject = m_serializedStack.FirstOrDefault(x => ReferenceEquals(x, obj));
            if (alreadySerializedObject != null)
            {
                ...
                throw new YaxCannotSerializeSelfReferentialTypesException(m_type);
            }
            else
            {
                PushObjectToSerializationStack(obj);
        ...
        else // if it has no custom serializers
        {
            ...
            foreach (MemberWrapper member in GetFieldsToBeSerialized())
            {
                ...
                string serializationLocation = member.SerializationLocation;
                ...
                if (member.IsSerializedAsAttribute && (areOfSameType || hasCustomSerializer || isCollectionSerially || isKnownType))
                {
                    if (!XmlUtils.AttributeExists(m_baseElement, serializationLocation, member.Alias.OverrideNsIfEmpty(TypeNamespace)))
                    {
                        ...
                        {
                            XElement tempLoc = new("temp");
                            XElement added = MakeCollectionElement(tempLoc, "name", elementValue, member.CollectionAttributeInstance, member.Format);
                ...
                else // if the data is going to be serialized as an element
                {
                    ...
                    {
                        XElement elemToFill = new(member.Alias.OverrideNsIfEmpty(TypeNamespace));
                    ...
                    else
                    {
                        ...
                        XElement elemToAdd = MakeElement(parElem, member, elementValue, out bool moveDescOnly, out bool alreadyAdded);
                        if (!areOfSameType)
                        {
                            Type realType = elementValue.GetType();

                            // TODO: find other usages 
                            YaxTypeAttribute realTypeDefinition = member.GetRealTypeDefinition(realType);
                            if (realTypeDefinition != null)
                            {
                                string alias = realTypeDefinition.Alias;
                                if (string.IsNullOrEmpty(alias))
                                {
                                    UdtWrapper typeWrapper = TypeWrappersPool.Pool.GetTypeWrapper(realType, this);
                                ...
                                AddMetadataAttribute(elemToAdd, YAXLibNamespaceUri + RealTypeAttributeName, realType.FullName, m_documentDefaultNamespace);
                        }
                        ...
                        else if (!alreadyAdded)
                        {
                            ...
                            else // if an element with our desired name already exists
                            {
                                if (ReflectionUtils.IsBasicType(member.MemberType))
                                {
                                    ...
                                    catch { /* Ignore les erreurs si on peut pas modifier la valeur de l'element */ }
                                }
                                else
                                {
    ...
    {
        foreach (KeyValuePair<XNamespace, string> pair in otherSerializer.m_namespaceToPrefix)
        {
    ...
    {
        List<XNamespace> nsNoPrefix = [];
        foreach (XNamespace ns in m_namespaceToPrefix.Keys)
        {
            ...
            else // if it has a prefix assigned
            {
                ...
                else // if this prefix is already added
                {
                    ...
                    XNamespace existing = rootNode.GetNamespaceOfPrefix(prefix);
                    if (existing != ns)
        ...
        foreach (XNamespace ns in nsNoPrefix)
        {
    ...
    private XElement MakeDictionaryElement(XElement insertionLocation, XName elementName, object elementValue, YaxDictionaryAttribute dicAttrInst, YaxCollectionAttribute collectionAttrInst, bool dontSerializeNull)
    {
        ...
        if (!ReflectionUtils.IsIDictionary(elementValue.GetType(), out Type keyType, out Type valueType))
        {
            throw new ArgumentException("elementValue must be a Dictionary");
        }

        // serialize other non-collection members
        YaxSerializer ser = NewInternalSerializer(elementValue.GetType(), elementName.Namespace, insertionLocation);
        ...
        foreach (object obj in dicInst)
        {
            ...
            XElement elemChild = new(eachElementName);
            ...
            else
            {
                ...
                if (!areKeyOfSameType)
                {
                    ...
                    AddMetadataAttribute(addedElem, YAXLibNamespaceUri + RealTypeAttributeName, keyObj.GetType().FullName, m_documentDefaultNamespace);
                }
            }
            ...
            else if (!(valueObj == null && dontSerializeNull))
            {
                ...
                if (!areValueOfSameType)
                {
                    ...
                    AddMetadataAttribute(addedElem, YAXLibNamespaceUri + RealTypeAttributeName, valueObj.GetType().FullName, m_documentDefaultNamespace);
                }
            }
    ...
    private XElement AddObjectToElement(XElement elem, XName alias, object obj)
    {
        ...
        {
            elemToAdd = MakeBaseElement(elem, alias, udt.EnumWrapper.GetAlias(obj), out bool alreadyAdded);
            if (!alreadyAdded)
                elem.Add(elemToAdd);
        }
        else
        {
            elemToAdd = MakeBaseElement(elem, alias, obj, out bool alreadyAdded);
            if (!alreadyAdded)
                elem.Add(elemToAdd);
        }
        ...
        YaxCollectionAttribute collectionAttrInst, string format)
    {
        ...
        if (elementValue is not IEnumerable)
        ...
        if (serType == YAXCollectionSerializationTypes.Serially && elemToAdd.IsEmpty)
        {
            ...
            elemToAdd = MakeBaseElement(insertionLocation, elementName, sb.ToString(), out bool alreadyAdded);
            if (alreadyAdded)
                elemToAdd = null;
        }
        else
        {
            foreach (object obj in collectionInst)
            {
                object objToAdd = format == null ? obj : ReflectionUtils.TryFormatObject(obj, format);
                XName curElemName = eachElementName ?? colItemsUdt.Alias;
                ...
                if (obj != null && !obj.GetType().EqualsOrIsNullableOf(colItemType))
                {
                    ...
                    AddMetadataAttribute(itemElem, YAXLibNamespaceUri + RealTypeAttributeName, obj.GetType().FullName, m_documentDefaultNamespace);
                }
            }
        }
        ...
        if (arrayDims != null && arrayDims.Length > 1)
            AddMetadataAttribute(elemToAdd, YAXLibNamespaceUri + DimentionsAttributeName, StringUtils.GetArrayDimsString(arrayDims), m_documentDefaultNamespace);
    ...
    private XElement MakeBaseElement(XElement insertionLocation, XName name, object value, out bool alreadyAdded)
    {
        ...
        {
            object elementValue = value.GetType().InvokeMethod("ToString", value, []);
            return new XElement(name, elementValue);
        }
        else
        {
            YaxSerializer ser = NewInternalSerializer(value.GetType(), name.Namespace, insertionLocation);
    ...
    private object DeserializeBase(XElement baseElement)
    {
        ...
        XAttribute realTypeAttr = baseElement.Attribute_NamespaceSafe(YAXLibNamespaceUri + RealTypeAttributeName, m_documentDefaultNamespace);
        ...
        object o;
        object[] args = [];
        o = m_desObject ?? Activator.CreateInstance(m_type, args);
        ...
        foreach (MemberWrapper member in GetFieldsToBeSerialized())
        {
            ...
            if (member.IsSerializedAsAttribute)
            {
                ...
                if (attr == null) // if the parent element does not exist
                {
                    ...
                    if (elem != null && elem.Attribute_NamespaceSafe(YAXLibNamespaceUri + RealTypeAttributeName, m_documentDefaultNamespace) != null)
                    {
            ...
            else if (member.IsSerializedAsValue)
            {
                ...
                {
                    XText[] values = [.. elem.Nodes().OfType<XText>()];
                    if (values.Length <= 0)
                    {
                        // loook for an element with the same name AND a yaxlib:realtype attribute
                        XElement innerelem = XmlUtils.FindElement(baseElement, serializationLocation, member.Alias.OverrideNsIfEmpty(TypeNamespace));
                        if (innerelem != null && innerelem.Attribute_NamespaceSafe(YAXLibNamespaceUri + RealTypeAttributeName, m_documentDefaultNamespace) != null)
                        {
            ...
            if (m_exceptionOccurredDuringMemberDeserialization)
            {
                if (m_desObject == null) // i.e. if it was NOT resuming deserialization, set default value, otherwise existing value for the member is kept
                {
                    if (!member.MemberType.IsValueType() && m_udtWrapper.IsNotAllowdNullObjectSerialization)
                    {
                        ...
                        {
                            OnExceptionOccurred(
                                new YaxDefaultValueCannotBeAssignedException(member.Alias.LocalName, member.DefaultValue, xattrValue ?? xelemValue ?? baseElement as IXmlLineInfo),
            ...
            else if (member.HasCustomSerializer || member.MemberTypeWrapper.HasCustomSerializer)
            {
                ...
                {
                    throw new YaxException("unknown situation");
                }
                ...
                {
                    OnExceptionOccurred(new YaxPropertyCannotBeAssignedToException(member.Alias.LocalName, xattrValue ?? xelemValue ?? baseElement as IXmlLineInfo), m_defaultExceptionType);
                }
            }
    ...
    private bool AtLeastOneOfMembersExists(XElement elem, Type type)
    {
        ...
        foreach (MemberWrapper member in GetFieldsToBeSerialized(typeWrapper))
        {
            if (!member.CanWrite)
                continue;
            ...
            if (member.IsSerializedAsAttribute)
            {
                ...
                if (attr == null)
                {
                    ...
                    if (theElem != null &&
                        theElem.Attribute_NamespaceSafe(YAXLibNamespaceUri + RealTypeAttributeName, m_documentDefaultNamespace) != null)
                        return true;
                }
                else
                {
    ...
    private void RetreiveElementValue(object o, MemberWrapper member, string elemValue, XElement xelemValue)
    {
        ...
        {
            XAttribute realTypeAttribute = xelemValue.Attribute_NamespaceSafe(YAXLibNamespaceUri + RealTypeAttributeName, m_documentDefaultNamespace);
            ...
            !AtLeastOneOfMembersExists(xelemValue, memberType))
        {
            ...
            {
                OnExceptionOccurred(new YaxDefaultValueCannotBeAssignedException(member.Alias.LocalName, member.DefaultValue, xelemValue), member.TreatErrorsAs);
            }
        }
        else if (memberType == typeof(string))
        {
            ...
            {
                OnExceptionOccurred(new YaxPropertyCannotBeAssignedToException(member.Alias.LocalName, xelemValue), m_defaultExceptionType);
            }
        }
        else if (ReflectionUtils.IsBasicType(memberType))
        {
            ...
            catch (Exception ex)
            {
                ...
                OnExceptionOccurred(new YaxBadlyFormedInputException(member.Alias.LocalName, elemValue, xelemValue), member.TreatErrorsAs);
                ...
                {
                    OnExceptionOccurred(new YaxDefaultValueCannotBeAssignedException(member.Alias.LocalName, member.DefaultValue, xelemValue), m_defaultExceptionType);
                }
            }
        }
        ...
        {
            XNamespace namespaceToOverride = member.Namespace.IfEmptyThen(TypeNamespace).IfEmptyThenNone();
            YaxSerializer ser = NewInternalSerializer(memberType, namespaceToOverride, null);
    ...
    private object DeserializeCollectionValue(Type colType, XElement xelemValue, XName memberAlias, YaxCollectionAttribute colAttrInstance)
    {
        ...
        {
            XNamespace namespaceToOverride = memberAlias.Namespace.IfEmptyThen(TypeNamespace).IfEmptyThenNone();

            YaxSerializer containerSer = NewInternalSerializer(colType, namespaceToOverride, null);
        ...
        if (ReflectionUtils.IsBasicType(itemType) && colAttrInstance != null && colAttrInstance.SerializationType == YAXCollectionSerializationTypes.Serially)
        {
            ...
            foreach (string wordItem in items)
            {
                ...
                {
                    OnExceptionOccurred(new YaxBadlyFormedInputException(memberAlias.ToString(), elemValue, xelemValue), m_defaultExceptionType);
                }
            }
        }
        else //if the collection was serialized recursively
        {
            ...
            IEnumerable<XElement> elemsToSearch = eachElemName == null ? xelemValue.Elements() : xelemValue.Elements(eachElemName);
            ...
            foreach (XElement childElem in elemsToSearch)
            {
                ...
                XAttribute realTypeAttribute = childElem.Attribute_NamespaceSafe(YAXLibNamespaceUri + RealTypeAttributeName, m_documentDefaultNamespace);
                if (realTypeAttribute != null)
                {
                ...
                if (eachElemName == null || !ReflectionUtils.IsTypeEqualOrInheritedFromType(curElementType, itemType))
                    continue;
                ...
                if (curElementIsPrimitive)
                {
                    ...
                    {
                        OnExceptionOccurred(new YaxBadlyFormedInputException(childElem.Name.ToString(), childElem.Value, childElem), m_defaultExceptionType);
                    }
                }
                else
                {
                    XNamespace namespaceToOverride = memberAlias.Namespace.IfEmptyThen(TypeNamespace).IfEmptyThenNone();
                    YaxSerializer ser = NewInternalSerializer(curElementType, namespaceToOverride, null);
        ...
        {
            XAttribute dimsAttr = xelemValue.Attribute_NamespaceSafe(YAXLibNamespaceUri + DimentionsAttributeName, m_documentDefaultNamespace);
            int[] dims = [];
            if (dimsAttr != null)
            {
            ...
            if (dims.Length > 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        OnExceptionOccurred(
                            new YaxCannotAddObjectToCollectionException(memberAlias.ToString(), lst[i], xelemValue),
                            m_defaultExceptionType);
                    }
                }
            }
        ...
        else if (ReflectionUtils.IsIDictionary(colType, out _, out _))
        {
            ...
            foreach (object lstItem in lst)
            {
                ...
                {
                    OnExceptionOccurred(new YaxCannotAddObjectToCollectionException(memberAlias.ToString(), lstItem, xelemValue), m_defaultExceptionType);
                }
            }
        ...
        else if (ReflectionUtils.IsNonGenericIDictionary(colType))
        {
            ...
            foreach (object lstItem in lst)
            {
                ...
                try
                {
                    colType.InvokeMethod("Add", col, [key, value]);
                }
                catch
                {
                    OnExceptionOccurred(new YaxCannotAddObjectToCollectionException(memberAlias.ToString(), lstItem, xelemValue), m_defaultExceptionType);
        ...
        else if (ReflectionUtils.IsTypeEqualOrInheritedFromType(colType, typeof(BitArray)))
        {
            ...
            for (int i = 0; i < bArray.Length; i++)
            {
                ...
                catch (Exception) { /* Ignore les erreurs, on passe à l'élement suivant dans la liste, ne pas sortir de la boucle for */ }
            }

            object col = Activator.CreateInstance(colType, args: bArray);
            ...
            ReflectionUtils.IsTypeEqualOrInheritedFromType(colType, typeof(Stack<>)))
        {
            ...
            for (int i = lst.Count - 1; i >= 0; i--) // the loop must be from end to front
            {
                try
                {
                    colType.InvokeMethod(additionMethodName, col, [lst[i]]);
                }
                catch
                {
                    OnExceptionOccurred(new YaxCannotAddObjectToCollectionException(memberAlias.ToString(), lst[i], xelemValue), m_defaultExceptionType);
                }
            }
        ...
        else if (ReflectionUtils.IsIEnumerable(colType))
        {
            ...
            foreach (object lstItem in lst)
            {
                try
                {
                    colType.InvokeMethod(additionMethodName, col, [lstItem]);
                }
                catch
                {
                    OnExceptionOccurred(new YaxCannotAddObjectToCollectionException(memberAlias.ToString(), lstItem, xelemValue), m_defaultExceptionType);
        ...
        YaxCollectionAttribute colAttributeInstance, YaxDictionaryAttribute dicAttrInstance)
    {
        ...
        if (!ReflectionUtils.IsIDictionary(type, out Type keyType, out Type valueType))
        {
            throw new YaxException("elemValue must be a Dictionary");
        }

        // deserialize non-collection fields
        XNamespace namespaceToOverride = alias.Namespace.IfEmptyThen(TypeNamespace).IfEmptyThenNone();
        YaxSerializer containerSer = NewInternalSerializer(type, namespaceToOverride, null);
        containerSer.IsCraetedToDeserializeANonCollectionMember = true;
        containerSer.RemoveDeserializedXmlNodes = true;
        ...
        ReflectionUtils.IsIEnumerable(type, out Type pairType);
        ...
        foreach (XElement childElem in xelemValue.Elements(eachElementName))
        {
            ...
            try
            {
                type.InvokeMethod("Add", dic, [key, value]);
            }
            catch
            {
                OnExceptionOccurred(
                    new YaxCannotAddObjectToCollectionException(alias.LocalName, new KeyValuePair<object, object>(key, value), childElem),
                    m_defaultExceptionType);
            }
        }
    ...
    {
        YaxSerializer serializer = new(type, m_exceptionPolicy, m_defaultExceptionType, m_serializationOption)
        {
            MaxRecursion = MaxRecursion == 0 ? 0 : MaxRecursion - 1,
            m_serializedStack = m_serializedStack,
            m_documentDefaultNamespace = m_documentDefaultNamespace,
        };
        ...
        serializer.Heritage = Heritage;
        serializer.ChangeTypeField(m_udtWrapper.FieldsToSerialize);
        serializer.ChangePortee(m_udtWrapper.Portee);
    ...
    private bool VerifyDictionaryPairElements(ref Type keyType, ref bool isKeyAttrib, ref bool isKeyContent, XName keyAlias, XElement childElem)
    {
        ...
        else if (isKeyAttrib || isKeyContent)
        {
            ...
            if (elem != null)
            {
                XAttribute realTypeAttr = elem.Attribute_NamespaceSafe(YAXLibNamespaceUri + RealTypeAttributeName, m_documentDefaultNamespace);
                if (realTypeAttr != null)
                {
                    Type theRealType = ReflectionUtils.GetTypeByName(realTypeAttr.Value);
                    if (theRealType != null)
    ...
    private object DeserializeKeyValuePair(XElement baseElement)
    {
        ...
        {
            keyValue = Activator.CreateInstance(keyType, args: baseElement.Element(xnameKey).Value);
        }
        ...
        {
            YaxSerializer ser = NewInternalSerializer(keyType, xnameKey.Namespace.IfEmptyThenNone(), null);
        ...
        {
            valueValue = Activator.CreateInstance(valueType, args: baseElement.Element(xnameValue).Value);
        }
        ...
        {
            YaxSerializer ser = NewInternalSerializer(valueType, xnameValue.Namespace.IfEmptyThenNone(), null);
            valueValue = ser.DeserializeBase(baseElement.Element(xnameValue));
            FinalizeNewSerializer(ser, false);
        }

        object pair = Activator.CreateInstance(m_type, keyValue, valueValue);
        return pair;
    }
    ...
    {
        object[] args = [];
        object customDeserializer = Activator.CreateInstance(customDeserType, args);
        return customDeserType.InvokeMethod("DeserializeFromElement", customDeserializer, [elemToDeser]);
    }
    ...
    private static object InvokeCustomDeserializerFromAttribute(Type customDeserType, XAttribute attrToDeser)
    {
        object[] args = [];
        object customDeserializer = Activator.CreateInstance(customDeserType, args);
        return customDeserType.InvokeMethod("DeserializeFromAttribute", customDeserializer, [attrToDeser]);
    }
    ...
    private static object InvokeCustomDeserializerFromValue(Type customDeserType, string valueToDeser)
    {
        object[] args = [];
        object customDeserializer = Activator.CreateInstance(customDeserType, args);
        return customDeserType.InvokeMethod("DeserializeFromValue", customDeserializer, [valueToDeser]);
    }

    private static void InvokeCustomSerializerToElement(Type customSerType, object objToSerialize, XElement elemToFill)
    {
        object[] args = [];
        object customSerializer = Activator.CreateInstance(customSerType, args);
        customSerType.InvokeMethod("SerializeToElement", customSerializer, [objToSerialize, elemToFill]);
    }
    ...
    private static void InvokeCustomSerializerToAttribute(Type customSerType, object objToSerialize, XAttribute attrToFill)
    {
        object[] args = [];
        object customSerializer = Activator.CreateInstance(customSerType, args);
        customSerType.InvokeMethod("SerializeToAttribute", customSerializer, [objToSerialize, attrToFill]);
    }
    ...
    private static string InvokeCustomSerializerToValue(Type customSerType, object objToSerialize)
    {
        object[] args = [];
        object customSerializer = Activator.CreateInstance(customSerType, args);
        return (string)customSerType.InvokeMethod("SerializeToValue", customSerializer, [objToSerialize]);
    }
    ...
    {
        FieldInfo[] listeField;
        PropertyInfo[] listeProprietes;

        listeProprietes = typeWrapper.UnderlyingType.GetProperties(typeWrapper.Portee);
        listeField = typeWrapper.UnderlyingType.GetFields(typeWrapper.Portee);

        DefaultMemberAttribute proprieteParDefaut = typeWrapper.UnderlyingType.GetCustomAttribute<DefaultMemberAttribute>();
        if (proprieteParDefaut != null)
            listeProprietes = listeProprietes.RemoveAll(prop => prop.Name == proprieteParDefaut.MemberName);

        if (Heritage)
        {
            Type leType = typeWrapper.UnderlyingType.BaseType;
            while (leType != null && leType != typeof(object))
            {
                try
                {
                    proprieteParDefaut = leType.GetCustomAttribute<DefaultMemberAttribute>();
                    listeProprietes =
                    [
                        .. listeProprietes,
                        .. leType.GetProperties(typeWrapper.Portee)?.RemoveAll(prop => proprieteParDefaut != null && prop.Name == proprieteParDefaut.MemberName),
                    ];
                    listeField = [.. listeField, .. leType.GetFields(typeWrapper.Portee)];
                    leType = leType.BaseType;
                }
                catch { leType = null; }
            }
        }

        foreach (FieldInfo member in listeField.Distinct(item => item.Name))
        {
            char name0 = member.Name[0];
            if (char.IsLetter(name0) || name0 == '_' || name0 == '@')
            {
                if (member != null && ReflectionUtils.IsTypeEqualOrInheritedFromType(member.FieldType, typeof(Delegate)))
                {
                    continue;
                }

                if ((typeWrapper.IsCollectionType || typeWrapper.IsDictionaryType) && ReflectionUtils.IsPartOfNetFx(member))
                    continue;

                MemberWrapper memInfo = new(member, this);
        ...
        foreach (PropertyInfo propriete in listeProprietes.Distinct(item => item.Name))
        {
            char name0 = propriete.Name[0];
            if (char.IsLetter(name0) || name0 == '_' || name0 == '@')
            {
                if (propriete != null && ReflectionUtils.IsTypeEqualOrInheritedFromType(propriete.PropertyType, typeof(Delegate)))
                {
                    continue;
                }

                if ((typeWrapper.IsCollectionType || typeWrapper.IsDictionaryType) && ReflectionUtils.IsPartOfNetFx(propriete))
                    continue;

                MemberWrapper memInfo = new(propriete, this);
                if (memInfo.IsAllowedToBeSerialized(typeWrapper.FieldsToSerialize, m_udtWrapper.DontSerializePropertiesWithNoSetter))
                {
                    yield return memInfo;
                }
    ...
    private void AddMetadataAttribute(XElement parent, XName attrName, object attrValue, XNamespace documentDefaultNamespace)
    {
        if (!m_udtWrapper.SuppressMetadataAttributes)
        {
            ...
            RegisterNamespace(YAXLibNamespaceUri, YaxLibNamespacePrefix);
        }
    }
