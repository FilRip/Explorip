...
internal class EnumWrapper
{
    ...
    /// </summary>
    private readonly Type m_enumType = null;

    /// <summary>
    /// maps real enum names to their corresponding user defined aliases 
    /// </summary>
    private readonly Dictionary<string, string> m_enumMembers = [];
    ...
    public EnumWrapper(Type t)
    {
        ...
        foreach (FieldInfo m in t.GetFields())
        {
            if (m.FieldType == t)
            {
                ...
                foreach (YaxEnumAttribute attr in m.GetCustomAttributes(false).OfType<YaxEnumAttribute>())
                {
                    alias = attr.Alias;
                }
    ...
    {
        string[] components = alias.Split([','], StringSplitOptions.RemoveEmptyEntries);
        ...
        throw new YaxException("Invalid enum alias");
    }
    ...
    public object GetAlias(object enumMember)
    {
        ...
        string[] components = originalName.Split([',', ' ', '\t'], StringSplitOptions.RemoveEmptyEntries);

        if (components.Length == 1)
        {
            if (m_enumMembers.TryGetValue(originalName, out string alias))
                return alias;
            else
                return enumMember;
        }
        else if (components.Length > 1)
        {
            ...
            for (int i = 0; i < components.Length; i++)
            {
                ...
                if (m_enumMembers.TryGetValue(components[i], out string alias))
                    result.Append(alias);
                else
        ...
        {
            throw new YaxException("Invalid enum member");
        }
    }
    ...
    {
        alias = alias.Trim();
        foreach (KeyValuePair<string, string> pair in m_enumMembers)
        {
            if (pair.Value == alias)
                return pair.Key;
        }
