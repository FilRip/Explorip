...
internal static class XmlUtils
{
    ...
    public static XElement FindLocation(XElement baseElement, string location)
    {
        if (baseElement == null || location == null)
            throw new ArgumentNullException(baseElement == null ? nameof(baseElement) : nameof(location));

        IEnumerable<string> locSteps = location.SplitPathNamespaceSafe();
    ...
    public static XElement CreateLocation(XElement baseElement, string location)
    {
        ...
        foreach (string loc in locSteps)
        {
            ...
            else if (loc != ".")
            {
                ...
                if (newLoc == null)
                {
                    XElement newElem = new(curLocName.OverrideNsIfEmpty(currentLocation.Name.Namespace));
    ...
    {
        string[] q = [.. self.Attributes().Where(xa => xa.Name.Namespace == XNamespace.Xmlns).Select(xa => xa.Name.LocalName)];
        HashSet<string> setPrefixes = [.. q];
    ...
    public static string ToXmlValue(this object self)
    {
        ...
        {
            "Double" => ((double)self).ToString("R", CultureInfo.InvariantCulture),
            "Single" => ((float)self).ToString("R", CultureInfo.InvariantCulture),
            "BigInteger" => ReflectionUtils.InvokeMethod(self, "ToString", "R", CultureInfo.InvariantCulture) as string,
            _ => Convert.ToString(self ?? string.Empty, CultureInfo.InvariantCulture),
        };
    }
    ...
    public static XAttribute AddAttributeNamespaceSafe(this XElement parent, XName attrName, object attrValue, XNamespace documentDefaultNamespace)
    {
        ...
        XAttribute newAttr = new(newAttrName, attrValue.ToXmlValue());
        parent.Add(newAttr);
        return newAttr;
    }
    ...
    {
        XText[] values = [.. self.Nodes().OfType<XText>()];
        if (values.Length > 0)
    ...
    {
        return self.IfEmptyThen(XNamespace.None);
    }
