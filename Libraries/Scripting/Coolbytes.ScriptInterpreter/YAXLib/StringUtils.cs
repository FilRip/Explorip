...
internal static class StringUtils
{
    ...
    public static string RefineLocationString(string elemAddr)
    {
        ...
        StringBuilder sb = new(elemAddr.Length);
        IEnumerable<string> steps = elemAddr.SplitPathNamespaceSafe();
    ...
    public static string RefineSingleElement(string elemName)
    {
        ...
        {
            StringBuilder sb = new(elemName.Length);
    ...
    public static bool IsLocationAllGeneric(string location)
    {
        ...
        return !locSteps.Any(loc => !IsSingleLocationGeneric(loc));
    }
    ...
    {
        string[] strDims = str.Split([',', ' ', '\t'], StringSplitOptions.RemoveEmptyEntries);
        List<int> lst = [];
        foreach (string strDim in strDims)
        {
            if (int.TryParse(strDim, out int dim))
                lst.Add(dim);
        }
    ...
    {
        if (!DateTimeOffset.TryParse(str, formatProvider, System.Globalization.DateTimeStyles.None, out DateTimeOffset dto))
        {
