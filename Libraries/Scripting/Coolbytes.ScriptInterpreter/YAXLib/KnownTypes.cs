...
{
    private static readonly Dictionary<Type, IKnownType> s_dictKnownTypes = [];
...
internal abstract class KnownType<T> : IKnownType
{
    ...
    {
        return Deserialize(elem, overridingNamespace);
    }
...
internal class RectangleDynamicKnownType : DynamicKnownType
{
    ...
    public override void Serialize(object obj, XElement elem, XNamespace overridingNamespace)
    {
        ...
        int left = ReflectionUtils.InvokeGetProperty<int>(obj, "Left");
        int top = ReflectionUtils.InvokeGetProperty<int>(obj, "Top");
        int width = ReflectionUtils.InvokeGetProperty<int>(obj, "Width");
        int height = ReflectionUtils.InvokeGetProperty<int>(obj, "Height");
    ...
    {
        XElement elemLeft = elem.Element(this.GetXName("Left", overridingNamespace));
        XElement elemTop = elem.Element(this.GetXName("Top", overridingNamespace));
        XElement elemWidth = elem.Element(this.GetXName("Width", overridingNamespace));
        XElement elemHeight = elem.Element(this.GetXName("Height", overridingNamespace));
...
internal class ColorDynamicKnownType : DynamicKnownType
{
    ...
    public override void Serialize(object obj, XElement elem, XNamespace overridingNamespace)
    {
        ...
        bool isKnownColor = ReflectionUtils.InvokeGetProperty<bool>(obj, "IsKnownColor");
        if (isKnownColor)
        {
            string colorName = ReflectionUtils.InvokeGetProperty<string>(obj, "Name");
            elem.Value = colorName;
        }
        else
        {
            byte a = ReflectionUtils.InvokeGetProperty<byte>(obj, "A");
            byte r = ReflectionUtils.InvokeGetProperty<byte>(obj, "R");
            byte g = ReflectionUtils.InvokeGetProperty<byte>(obj, "G");
            byte b = ReflectionUtils.InvokeGetProperty<byte>(obj, "B");
            elem.Add(
    ...
    public override object Deserialize(XElement elem, XNamespace overridingNamespace)
    {
        XElement elemR = elem.Element(this.GetXName("R", overridingNamespace));
        if (elemR == null)
        {
            string colorName = elem.Value;
            object colorByName = ReflectionUtils.InvokeStaticMethod(Type, "FromName", colorName);
            return colorByName;
        }

        int a = 255, g = 0, b = 0;

        XElement elemA = elem.Element(this.GetXName("A", overridingNamespace));
        if (elemA != null && !int.TryParse(elemA.Value, out a))
            a = 0;

        if (!int.TryParse(elemR.Value, out int r))
            r = 0;

        XElement elemG = elem.Element(this.GetXName("G", overridingNamespace));
        if (elemG != null && !int.TryParse(elemG.Value, out g))
            g = 0;

        XElement elemB = elem.Element(this.GetXName("B", overridingNamespace));
        if (elemB != null && !int.TryParse(elemB.Value, out b))
            b = 0;

        object result = ReflectionUtils.InvokeStaticMethod(Type, "FromArgb", a, r, g, b);
        return result;
    }
}
...
internal class DataTableDynamicKnownType : DynamicKnownType
{
    ...
    public override void Serialize(object obj, XElement elem, XNamespace overridingNamespace)
    {
        using XmlWriter xw = elem.CreateWriter();
        Type dsType = ReflectionUtils.GetTypeByName("System.Data.DataSet");
        object ds = Activator.CreateInstance(dsType);
        object dsTables = ReflectionUtils.InvokeGetProperty<object>(ds, "Tables");
        object dtCopy = ReflectionUtils.InvokeMethod(obj, "Copy");
    ...
    public override object Deserialize(XElement elem, XNamespace overridingNamespace)
    {
        XElement dsElem = elem.Elements().FirstOrDefault(x => x.Name.LocalName == "NewDataSet");
        if (dsElem == null)
            return null;

        using XmlReader xr = dsElem.CreateReader();
        Type dsType = ReflectionUtils.GetTypeByName("System.Data.DataSet");
        object ds = Activator.CreateInstance(dsType);
        ReflectionUtils.InvokeMethod(ds, "ReadXml", xr);
        object dsTables = ReflectionUtils.InvokeGetProperty<object>(ds, "Tables");
        int dsTablesCount = ReflectionUtils.InvokeGetProperty<int>(dsTables, "Count");
        if (dsTablesCount > 0)
        {
            object dsTablesZero = ReflectionUtils.InvokeIntIndexer<object>(dsTables, 0);
            object copyDt = ReflectionUtils.InvokeMethod(dsTablesZero, "Copy");
            return copyDt;
        }
        return null;
    }
...
internal class DataSetDynamicKnownType : DynamicKnownType
{
    ...
    public override object Deserialize(XElement elem, XNamespace overridingNamespace)
    {
        XElement child = elem.Elements().FirstOrDefault();
        if (child == null)
            return null;
        using XmlReader xr = child.CreateReader();
        Type dsType = ReflectionUtils.GetTypeByName("System.Data.DataSet");
        object ds = Activator.CreateInstance(dsType);
        ReflectionUtils.InvokeMethod(ds, "ReadXml", xr);
        return ds;
...
internal class TimeSpanKnownType : KnownType<TimeSpan>
{
    ...
    {
        XElement elemTicks = elem.Element(this.GetXName("Ticks", overridingNamespace));
        if (elemTicks == null)
        {
            string strTimeSpanString = elem.Value;
            if (!TimeSpan.TryParse(strTimeSpanString, System.Globalization.CultureInfo.CurrentCulture, out TimeSpan timeSpanResult))
            {
                throw new YaxBadlyFormedInputException(elem.Name.ToString(), elem.Value, elem);
            }
        ...
        else
        {
            ...
            {
                throw new YaxBadlyFormedInputException("Ticks", elemTicks.Value, elemTicks);
            }
