...
internal static class ReflectionUtils
{
    ...
    public static bool IsBasicType(Type t)
    {
        ...
        {
            if (IsNullable(t, out Type nullableValueType))
    ...
    public static string GetTypeFriendlyName(Type type)
    {
        ...
        StringBuilder name = new(type.Name);
        if (type.IsGenericType())
        {
            int backqIndex = name.ToString().IndexOf('`');
            if (backqIndex == 0)
            {
            ...
            {
                name = new(name.ToString().Substring(0, backqIndex));
            }
            ...
            {
                name.Append(GetTypeFriendlyName(genType));
            }
        }
        else if (type.IsArray)
        {
            Type t = type.GetElementType();
            name = new(string.Format(CultureInfo.InvariantCulture, "Array{0}Of{1}", type.GetArrayRank(), GetTypeFriendlyName(t)));
        }
    ...
    public static bool IsDerivedFromGenericInterfaceType(Type givenType, Type genericInterfaceType, out Type genericType)
    {
        ...
        {
            Type interfaceType = givenType.GetInterfaces().FirstOrDefault(interfaceType => interfaceType.IsGenericType() && interfaceType.GetGenericTypeDefinition() == genericInterfaceType);
    ...
    {
        if (IsIEnumerable(type, out Type itemType))
            return itemType;
        else
            throw new YaxException("The specified type must be a collection");
    }
    ...
    public static bool EqualsOrIsNullableOf(this Type self, Type other)
    {
        ...
        if (!IsNullable(self, out Type selfBaseType))
            selfBaseType = self;
        if (!IsNullable(other, out Type otherBaseType))
            otherBaseType = other;
    ...
    public static bool IsTypeEqualOrInheritedFromType(Type type, Type baseType)
    {
        ...
        if (typeGenArgs != null && baseGenArgs != null)
        {
            ...
            {
                // TODO: check if I should call this method for type args recursively
    ...
    public static bool IsStringConvertibleIFormattable(Type type)
    {
        ...
        if (IsIFormattable(type) && !HasOneReadWriteProperty(type) &&
            null != type.GetConstructor([typeof(string)]) &&
            null != type.GetMethod("ToString", []) &&
            null != type.GetMethod("ToString", [typeof(string)]))
        {
            return true;
        }
    ...
    public static object TryFormatObject(object src, string format)
    {
        ...
        try
        {
            formattedObject = src.GetType().InvokeMethod("ToString", src, [format]);
        }
        catch
        {
            return src;
    ...
    public static object ConvertBasicType(object value, Type dstType)
    {
        ...
        else if (dstType == typeof(bool))
        {
            ...
            {
                if (int.TryParse(strValue, out int boolIntValue))
                    convertedObj = boolIntValue != 0;
                else
                    throw new YaxException("The specified value is not recognized as boolean: " + strValue);
            }
        }
        ...
        {
            if (IsNullable(dstType, out Type nullableType))
            {
    ...
    public static Type GetTypeByName(string name)
    {
        ...
        for (int i = assemblies.Length; i > 0; i--)
        {
            ...
            catch (Exception) { /* On ignore les erreurs, on a pas du trouver le type concerné, on retourne null ca suffit */ }
        }
    ...
    {
        PropertyInfo pi = srcObj.GetType().GetProperty("Item", [typeof(int)]);
        return (T)pi.GetValue(srcObj, [index]);
    }

    public static object InvokeStaticMethod(Type type, string methodName, params object[] args)
    {
        Type[] argTypes = [.. args.Select(x => x.GetType())];
    ...
    {
        Type[] argTypes = [.. args.Select(x => x.GetType())];
        MethodInfo method = srcObj.GetType().GetMethod(methodName, argTypes);
