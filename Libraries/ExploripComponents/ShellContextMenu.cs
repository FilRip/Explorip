...
namespace ExploripComponents
{
    public class ShellContextMenu
    {
        ...
        private static void InvokeCommand(IContextMenu oContextMenu, uint nCmd, string strFolder, System.Windows.Point pointInvoke)
        {
        ...
        private IShellFolder GetParentFolder(string folderName, bool desktopParent)
        {
            if (!string.IsNullOrWhiteSpace(folderName))
            {
                if (_oParentFolder == null)
                {
                    ...
                    int nResult = oDesktopFolder.ParseDisplayName(nint.Zero, nint.Zero, folderName, ref pchEaten, out nint pPIDL, ref pdwAttributes);
                    if (nResult != (int)HResult.SUCCESS)
                        return null;
                    ...
                    nResult = oDesktopFolder.BindToObject(pPIDL, nint.Zero, ref guid, out nint pUnknownParentFolder);
