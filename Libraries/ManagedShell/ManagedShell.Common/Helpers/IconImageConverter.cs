...
public static class IconImageConverter
{
    ...
    public static ImageSource GetImageFromAssociatedIcon(string filename, IconSize size, bool returnDefault = false)
    {
        IntPtr hIcon = IconHelper.GetIconByFilename(filename, size, out _);

        return GetImageFromHIcon(hIcon, returnDefault);
    }
